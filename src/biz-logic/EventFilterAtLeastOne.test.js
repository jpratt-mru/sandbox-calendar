...
let shouldBeFalse = function(filterText) {
  actualResult = eventFilter.eventMatchesAnyFilterText(
    EVENT_TO_FILTER,
    filterText
  );
...
describe("reports no match", () => {
  test("when using a null event", () => {
    let no_such_event;
    expect(
      eventFilter.eventMatchesAnyFilterText(no_such_event, "bar")
    ).toBeFalsy();
  });
