...
function processDroppedFile(file, courses, props) {
    ...
    reader.onload = () => {
        ...
        cleanedSpreadsheetRows.forEach(row => {
            let course = courseFactory.createSection(row, id);
            courses.push(course);
        });
        props.handleFileDrop(courses);
    };
...
function onDrop(props, acceptedFiles) {
    ...
    acceptedFiles.forEach(file => {
        processDroppedFile(file, courses, props);
    });
}
